...
1.A player looses hi ENTIRE score when he rolls  two 6 in a row. After that , it's the next player's turn.
...
3. Add another dice to the game , so that there are two dices now. The player looses his current score when one of them 
    is 1. (HINT : you will need CSS to position the second dices,so have a look at CSS for the first one.)
*/
...
document.querySelector('.btn-roll').addEventListener('click',function(){
    ...
    if(gamePlaying){
        ...
        // 2. Display the result
         ...
         if (dice1 !== 1 && dice2 !==1){                                            // !== different operator (does not use type coercion)
            // Add Score
             roundScore += (dice1+dice2);                                      // roundscore =roundscor+dice
